...
public class ReplyDAO {
	...
	public int addcomment(ReplyDTO dto) {
	...
	public List<ReplyDTO> selectByParentSeq(int Parent_seq) {
	...
	public ReplyDTO selectOne(int seq) {
		String sql = "select * from reply where seq = ?";
		
		return jdbc.queryForObject(sql, new BeanPropertyRowMapper<>(ReplyDTO.class), seq);
	}
	...
	public int updatecomment(ReplyDTO dto) {
		String sql = "update reply set contents = ? where seq = ?";
		
		return jdbc.update(sql, dto.getContents(), dto.getSeq());
	}
}
